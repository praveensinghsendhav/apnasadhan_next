    ...
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative hidden sm:block">
      <li>
        <Link
          ...
          href="#"
          className="relative flex h-12 w-12 items-center justify-center rounded-full border border-stroke bg-gray-2 text-dark hover:text-primary dark:border-dark-4 dark:bg-dark-3 dark:text-white dark:hover:text-white transition duration-300 ease-in-out shadow-md hover:shadow-lg"
        >
          <span className="relative">
            ...
            <span
              className={`absolute -top-0.5 right-0 z-1 h-2.5 w-2.5 rounded-full bg-red-light dark:border-dark-3 ${!notifying ? "hidden" : "inline"
                }`}
            >
        ...
        {dropdownOpen && (
          <div
            className={`absolute -right-27 mt-7.5 flex h-[550px] w-75 flex-col rounded-xl border-[0.5px] border-stroke bg-white px-5.5 pb-5.5 pt-5 shadow-lg dark:border-dark-3 dark:bg-gray-dark sm:right-0 sm:w-[364px] transition-all duration-300 ease-in-out transform ${dropdownOpen ? "scale-100 opacity-100" : "scale-95 opacity-0"
              }`}
          >
            ...
            <ul className="no-scrollbar mb-5 flex h-auto flex-col gap-1 overflow-y-auto overflow-x-hidden">
              {notificationList.map((item, index) => (
                <li key={index}>
                  <Link
                    className="flex items-center gap-4 rounded-[10px] p-2.5 hover:bg-gray-2 dark:hover:bg-dark-3 transition duration-300 ease-in-out transform hover:scale-105"
                    href="#"
                  >
            ...
            <Link
              className="flex items-center justify-center rounded-[7px] border border-primary p-2.5 font-medium text-primary hover:bg-blue-light-5 dark:border-dark-4 dark:text-dark-6 dark:hover:border-primary dark:hover:bg-blue-light-3 dark:hover:text-primary transition duration-300 ease-in-out"
              href="#"
            >
